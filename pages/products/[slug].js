      ...
      <div className="mx-auto mt-16 max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        <div className="mx-auto flex flex-col sm:flex-row">
          ...
          <div className="mt-10 flex flex-col sm:mt-0 sm:ml-10">
            ...
            <label
              className="mt-10 text-4xl font-bold text-gray-500 sm:text-3xl sm:tracking-tight lg:text-2xl"
              htmlFor="quantity"
            >
              Choose Quantity
            </label>
            <input
              className="text-black-500 mt-3 text-4xl font-bold sm:text-2xl"
              type="number"
              id="quantity"
              min={1}
              name="quantity"
            ...
            <h1 className="text-black-500 mt-5 text-4xl font-bold sm:text-2xl">
              Total paid: ${product.discountprice * quantity}
            </h1>
...
export async function getStaticProps({ params }) {
  const product = await axios.get(
    process.env.NEXT_PUBLIC_SERVER_URL + '/product/' + params.slug
  )
  return {
    props: {
